...
const BaseMultiSelect = ({ value = [], onChange, options, isSearch, style, selectStyle }) => {
  ...
  const [selectLocation, setSelectLocation] = useState({ left: 0, top: 0 });
  ...
  useEffect(() => {
    const blurFocus = (e) => {
      if (!selectRef.current.contains(e.target) && !inputRef.current.contains(e.target))
        setIsOpen(false);
    };
